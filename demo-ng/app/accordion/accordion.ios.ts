...
function notifyForItemAtIndex(owner, cell: any, view: View, eventName: string, indexPath: NSIndexPath) {
    let args = { eventName: eventName, object: owner, parentIndex: indexPath.row, childIndex: indexPath.section, view: view, ios: cell, android: undefined };
    owner.notify(args);
    return args;
}

function notifyForHeaderOrFooterAtIndex(owner, nativeView: any, view: View, eventName: string, parentIndex: number) {
...
function notifyForItemStartAtIndex(owner, eventName: string, indexPath: NSIndexPath) {
    let args = { eventName: eventName, object: owner, parentIndex: indexPath.row, childIndex: indexPath.section };
    owner.notify(args);
    return args;
}
...
export class AccordionDataSource extends NSObject implements UITableViewDataSource {
    ...
    public tableViewCellForRowAtIndexPath(tableView: UITableView, indexPath: NSIndexPath) {
        let owner = this._owner.get();
        let _args = notifyForItemStartAtIndex(owner, STARTITEMLOADING, indexPath);
        ...
        if (owner) {
            ...
            if (has && !selected) {
                ...
                if (!view) {
                    if ((<any>owner.itemTemplate) instanceof View) {
                        view = owner.itemTemplate;
                    }
                }
                if (view) {
                    ...
                    let args = notifyForItemAtIndex(owner, cell, view, ITEMSLOADING, indexPath);
                    ...
                    // console.log(view._nativeView.frame.size.height)
                    // console.log(view._nativeView.frame.size.width)
                    if (!view.parent) {
                        owner._addView(view);
                    }
                }
...
export class UITableViewDelegateImpl extends NSObject implements UITableViewDelegate {
    ...
    public tableViewViewForHeaderInSection(tableView: UITableView, section: number) {
        let owner = this._owner.get();
        let _args = notifyForHeaderOrFooterStartAtIndex(owner, STARTHEADERLOADING, section);
        ...
        if (view) {
            ...
            let args = notifyForHeaderOrFooterAtIndex(owner, view._nativeView, view, HEADERLOADING, section);
        ...
        let args = notifyForHeaderOrFooterAtIndex(owner, hv, null, HEADERLOADING, section);
        return hv;
    }
    public tableViewViewForFooterInSection(tableView: UITableView, section: number) {
        let owner = this._owner.get();
        let _args = notifyForHeaderOrFooterStartAtIndex(owner, STARTFOOTERLOADING, section);
        ...
        if (view) {
            ...
            let args = notifyForHeaderOrFooterAtIndex(owner, view._nativeView, view, FOOTERLOADING, section);
            return view._nativeView;
        }
